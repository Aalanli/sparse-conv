...
/*
there are two types of sparse convolution: sub-manifold and regular, subm
convolution does not change the number of coordinates, while regular convolution
does. Intuitively, subm convolution convolves the filter only at the center of
...
extern "C" int64_t
generate_conv3d_indices_cpu(const int *coords, // (N, 4)
                            int *new_coords,   // (N', 4)
                            int *indices,      // (N', K ** 3)
                            const int64_t N,   // number of coordinates
                            const int64_t K,   // kernel size
                            const int3 stride, const int3 pad,
                            const int3 bounds) {
    ...
    for (int64_t i = 0; i < N; ++i) {
        ...
        // y[b, i, j, k] = x[b, i * sx + dx - pad, j * sy + dy - pad, k * sz +
        // dz - pad] * w[dx, dy, dz] exists i, j, k such that i * sx + dx - pad
        // == x, etc.
        for (int dx = 0; dx < K; ++dx) {
            for (int dy = 0; dy < K; ++dy) {
                for (int dz = 0; dz < K; ++dz) {
                    bool exists = true;
                    exists &=
                        (x + pad.x - dx) % stride.x == 0 && x + pad.x - dx >= 0;
                    exists &=
                        (y + pad.y - dy) % stride.y == 0 && y + pad.y - dy >= 0;
                    exists &=
                        (z + pad.z - dz) % stride.z == 0 && z + pad.z - dz >= 0;
                    if (!exists)
                        continue;
                    ...
                    exists &= !(new_x < 0 || new_y < 0 || new_z < 0 ||
                                new_x >= bounds.x || new_y >= bounds.y ||
                                new_z >= bounds.z);
                    if (!exists) {
...
extern "C" void
generate_conv3d_subm_indices_cpu(const int *coords, // (N, 4)
                                 int *indices,      // (N, K ** 3)
            ...
            for (int dy = 0; dy < K; ++dy) {
                for (int dz = 0; dz < K; ++dz) {
                    ...
                    if (coord_map.find(packed_new_coord) != coord_map.end()) {
                        indices[i * K * K * K + offset] =
                            coord_map[packed_new_coord];
                    } else {
...
constexpr unsigned long long EMPTY_HASH =
    0xffffffffffffffffULL; // -1 in unsigned long long
...
__global__ void insert_indices_hash_kernel(const int *coords,  // (N, 4)
    ...
    for (int l = 0; l < max_lookup; ++l) {
        auto hkey = atomicCAS((unsigned long long *)hash_key + hash_index,
                              EMPTY_HASH, packed_coord);
        if (hkey == packed_coord) {
            return;
        } else if (hkey == EMPTY_HASH) {
            // if there are duplicate indices, then this becomes
            // non-deterministic
...
__global__ void
generate_conv3d_subm_indices_kernel(const int *coords,
                                    uint64_t *hash_key, // (T,)
                                    int *hash_value,    // (T,)
                                    int *indices,       // (N, K ** 3)
    ...
    for (int l = 0; l < lookup_tries; l++) {
        ...
        if (hash_key_val == packed_new_coord) { // found
            ...
            // printf("Not found idx: %d at hash_index: %d for packed_new_coord:
            // %llu\n", idx, hash_index, packed_new_coord); indices[idx * K * K
            // * K + offset] = -1;  // Not found
            return;
        }
        hash_index = (hash_index + 1) % T; // Linear probing
    }
...
extern "C" void
generate_conv3d_subm_indices_gpu(const int *coords,   // (N, 4)
                                 int64_t *hash_key,   // (T,)
                                 int *hash_value,     // (T,)
                                 int *indices,        // (N, K ** 3)
                                 ...
                                 int N,               // number of coordinates
                                 int K,               // kernel size
                                 int T, int lookup_tries, int threads,
                                 CUstream stream) {
    int threads_per_block = threads;
    int blocks = (N + threads_per_block - 1) / threads_per_block;
    insert_indices_hash_kernel<<<blocks, threads_per_block, 0, stream>>>(
        coords, (uint64_t *)hash_key, hash_value, N, T, lookup_tries,
        dropped_points);
    int blocks_subm =
        (N * K * K * K + threads_per_block - 1) / threads_per_block;
    generate_conv3d_subm_indices_kernel<<<blocks_subm, threads_per_block, 0,
                                          stream>>>(
        coords, (uint64_t *)hash_key, hash_value, indices, N, T, K,
        lookup_tries, dropped_points + 1);
}

template <typename F> __device__ void printv(F func) {
...
__global__ void generate_conv3d_indices_kernel_one_stage(
    const int *coords, // (N, 4)
    int *new_coords,   // (N', 4)
    int *indices,      // (N', K * K * K)
    ...
    int new_y = (y + pad.y - dy) / stride.y;
    int new_z = (z + pad.z - dz) / stride.z;
    exists &= !(new_x < 0 || new_y < 0 || new_z < 0 || new_x >= bounds.x ||
                new_y >= bounds.y || new_z >= bounds.z);
    // this boundary condition may drop points unexpectedly (without the correct
    // pad) this is to replicate torchsparse behavior
    exists &= new_x * stride.x + K - 1 < bounds.x + 2 * pad.x;
    exists &= new_y * stride.y + K - 1 < bounds.y + 2 * pad.y;
    exists &= new_z * stride.z + K - 1 < bounds.z + 2 * pad.z;
    // printv([=]() {printf("exists %d (%d, %d, %d)", exists, new_x, new_y,
    // new_z);});
    int new_coord[4] = {b, new_x, new_y, new_z};
    auto packed_new_coord = pack_coords_device(new_coord);
    ...
    if (exists) { // skip for all items that does not exist
        ...
        while (lookups < lookup_tries) {
            hkey = atomicCAS((unsigned long long *)hash_keys + hash_idx,
                             EMPTY_HASH, packed_new_coord);
    ...
    if (threadIdx.x == blockDim.x - 1) {
        local_offset_shared =
            atomicAdd(global_offset, offset_final + local_offset);
    }
    __syncthreads();
    ...
    // printv([=]() {printf("offset_final %d, idx  %d, offset %d", offset_final,
    // idx, offset);});
...
extern "C" void
generate_conv3d_indices_kernel_gpu(const int *coords, // (N, 4)
                                   int *new_coords,   // (N', 4)
                                   int *indices,      // (N', K * K * K)
                                   ...
                                   const int Nprime,
                                   const int K, // kernel size
                                   const int T, const int3 stride,
                                   const int3 pad, const int3 bounds,
                                   const int lookup_tries,
    ...
    case 128:
        generate_conv3d_indices_kernel_one_stage<128>
            <<<blocks, threads, 0, stream>>>(
                coords, new_coords, indices, global_offset, hash_keys,
                hash_values, N, Nprime, K, T, stride, pad, bounds, lookup_tries,
                dropped_points);
        break;
    case 256:
...
torch::Tensor
generate_conv3d_subm_indices(const torch::Tensor &coords, // (N, 4)
                             int64_t K,                   // kernel size
                             double hash_table_multiplier, int64_t threads,
                             int64_t lookup_tries) {
    TORCH_CHECK(coords.dim() == 2 && coords.size(1) == 4,
                "coords must be of shape (N, 4)");
    TORCH_CHECK(coords.dtype() == torch::kInt32,
                "coords must be of dtype int32");
    ...
    if (coords.device().is_cuda()) {
        ...
        TORCH_CHECK(hash_table_multiplier > 1,
                    "Hash table multiplier must be > 1");
        TORCH_CHECK(
            T > N * 2,
            "Hash table size must be greater than number of coordinates * 2");

        auto dropped_points = torch::zeros({2}, torch::TensorOptions()
                                                    .dtype(torch::kInt32)
                                                    .device(coords.device()));
        indices.fill_(-1);
        auto hash_keys = torch::empty({T}, torch::TensorOptions()
                                               .dtype(torch::kInt64)
                                               .device(coords.device()));
        hash_keys.fill_(-1);
        auto hash_values = torch::empty({T}, torch::TensorOptions()
                                                 .dtype(torch::kInt32)
                                                 .device(coords.device()));

        generate_conv3d_subm_indices_gpu(
            coords.data_ptr<int>(), hash_keys.data_ptr<int64_t>(),
            hash_values.data_ptr<int>(), indices.data_ptr<int>(),
            dropped_points.data_ptr<int>(), N, K, T, lookup_tries, threads,
            at::cuda::getCurrentCUDAStream().stream());
        if (dropped_points[0].item<int>() != 0 ||
            dropped_points[1].item<int>() != 0) {
            TORCH_CHECK(false, "Error: dropped points detected in "
                               "generate_conv3d_subm_indices "
                               "(hash table overflow or collision).");
        }
    } else {
        generate_conv3d_subm_indices_cpu(coords.data_ptr<int>(),
                                         indices.data_ptr<int>(), N, K);
    }
...
std::tuple<torch::Tensor, torch::Tensor> generate_conv3d_indices(
    ...
    const int64_t batch_size, const int64_t K, const int64_t stride_x,
    const int64_t stride_y, const int64_t stride_z, const int64_t pad_x,
    const int64_t pad_y, const int64_t pad_z, const int64_t max_x,
    const int64_t max_y, const int64_t max_z, double hash_table_multiplier,
    int64_t threads, int64_t lookup_tries) {
    TORCH_CHECK(coords.dim() == 2 && coords.size(1) == 4,
                "coords must be of shape (N, 4)");
    TORCH_CHECK(coords.dtype() == torch::kInt32,
                "coords must be of dtype int32");
    ...
    TORCH_CHECK(max_x >= 0 && max_y >= 0 && max_z >= 0,
                "Max coordinates must be non-negative");
    ...
    // number of additional points each non-zero point can theoretically add,
    // alone each dimension
    ...
    int NPrime = std::min(N * num_Kx * num_Ky * num_Kz,
                          num_x * num_y * num_z * batch_size);
    ...
    int3 stride = {static_cast<int>(stride_x), static_cast<int>(stride_y),
                   static_cast<int>(stride_z)};
    int3 pad = {static_cast<int>(pad_x), static_cast<int>(pad_y),
                static_cast<int>(pad_z)};
    int3 max_coords = {static_cast<int>(max_x), static_cast<int>(max_y),
                       static_cast<int>(max_z)};
    ...
    if (coords.device().is_cuda()) {
        ...
        // TORCH_CHECK(hash_table_multiplier > 1, "Hash table multiplier must be
        // > 1"); TORCH_CHECK(T > NPrime, "Hash table size must be greater than
        // number of coordinates * 2");

        auto global_offset = torch::empty({2}, torch::TensorOptions()
                                                   .dtype(torch::kInt32)
                                                   .device(coords.device()));
        ...
        auto hash_values = torch::empty({T}, torch::TensorOptions()
                                                 ...
                                                 .device(coords.device()));
        hash_values.fill_(-1);
        generate_conv3d_indices_kernel_gpu(
            coords.data_ptr<int>(), new_coords.data_ptr<int>(),
            indices.data_ptr<int>(),

            global_offset.data_ptr<int>(),
            (uint64_t *)hash_keys.data_ptr<int64_t>(),
            hash_values.data_ptr<int>(), N, NPrime, K, T, stride, pad,
            max_coords, lookup_tries,
        ...
        if (drooped_points > 0) {
            TORCH_CHECK(
                false,
                "Error: dropped points detected in generate_conv3d_indices "
                "(hash table overflow or collision).",
        ...
        int64_t NPrime = generate_conv3d_indices_cpu(
            coords.data_ptr<int>(), new_coords.data_ptr<int>(),
