...
#define CHECK(call)                                                            \
    do {                                                                       \
        ...
        if (_e != CUDA_SUCCESS) {                                              \
            ...
            std::cerr << "CUDA Error: " << err << " – " << str << " at line "  \
                      << __LINE__ << std::endl;                                \
            std::exit(EXIT_FAILURE);                                           \
        }                                                                      \
    } while (0)

Conv3DImplicitGemmKernel::Conv3DImplicitGemmKernel(json config,
                                                   std::string ptx_path) {
    ...
    CHECK(cuDeviceGetAttribute(
        &device_major, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, cuDevice));
    CHECK(cuDeviceGetAttribute(
        &device_minor, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, cuDevice));
    int device_sm = device_major * 10 + device_minor;

    valid = (device_sm == sm);
    if (!valid) {
        std::cerr << "Device SM " << device_sm
                  << " is not equal to the required SM " << sm
                  << ". Kernel will not be loaded from " << ptx_path
                  << std::endl;
        return;
    }
    ...
    this->ptx = std::string((std::istreambuf_iterator<char>(ptx_file)),
...
void Conv3DImplicitGemmKernel::run(CUdeviceptr features, // [N, D]
                                   ...
                                   int N, int NPrime, int D, int DPrime, int K,
                                   CUstream stream) {
    assert((!div_k || K % 16 == 0) &&
           "K must be divisible by 16 if div_k is true");
    assert((!div_d || D % 16 == 0) &&
           "D must be divisible by 16 if div_d is true");
    assert((!div_dp || DPrime % 16 == 0) &&
           "DPrime must be divisible by 16 if div_dp is true");

    int threads = num_warps * 32;
    int blocks = ((NPrime + block_n - 1) / block_n) *
                 ((DPrime + block_dp - 1) / block_dp);
...
setup(std::string ptx_dir) {
    ...
    for (auto &[key, value] : config.items()) {
        ...
        auto ker = new Conv3DImplicitGemmKernel(value, ptx_path);
