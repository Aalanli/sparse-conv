...
def implicit_gemm_idx_sort_kernel(
    ...
    inds_density_mask = (inds >= 0) & (inds < NReal)
...
def implicit_gemm_mask_kernel(
    ...
    for i in range(0, K3, BLOCK_K):
        ...
        imask = tl.reduce((inds < NReal) & (inds >= 0), 1, or_combine)  # [BLOCK_K]
...
@triton.autotune(
    configs=[
        ...
        # small dimensions
        triton.Config(
            {"BLOCK_K": 16, "BLOCK_Dp": 16, "PARALLEL_K": 1},
            num_warps=2,
            num_stages=2,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
        triton.Config(
            {"BLOCK_K": 16, "BLOCK_Dp": 32, "PARALLEL_K": 1},
            num_warps=2,
            num_stages=2,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
        # medium dimensions
        triton.Config(
            {"BLOCK_K": 32, "BLOCK_Dp": 64, "PARALLEL_K": 1},
            num_warps=2,
            num_stages=2,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
        ...
        triton.Config(
            {"BLOCK_K": 32, "BLOCK_Dp": 64, "PARALLEL_K": 2},
            num_warps=4,
            num_stages=3,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
        triton.Config(
            {"BLOCK_K": 32, "BLOCK_Dp": 32, "PARALLEL_K": 1},
            num_warps=2,
            num_stages=2,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
        ...
        triton.Config(
            {"BLOCK_K": 32, "BLOCK_Dp": 32, "PARALLEL_K": 2},
            num_warps=4,
            num_stages=3,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
        # large dimensions
        triton.Config(
            {"BLOCK_K": 16, "BLOCK_Dp": 128, "PARALLEL_K": 1},
            num_warps=4,
            num_stages=2,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
        ...
        triton.Config(
            {"BLOCK_K": 64, "BLOCK_Dp": 64, "PARALLEL_K": 1},
            num_warps=4,
            num_stages=2,
            pre_hook=implicit_gemm_conv3d_prehook,
        ),
...
def implicit_conv3d_kernel_T(
    ...
    for k_it in range(pid_k, stride_k, PARALLEL_K):
        ...
        if mask_i:
            # tl.device_print("inds", inds)
            inds = tl.load(
                ind_ptr + k * N_prime_stride, mask=(tl.arange(0, BLOCK_N) + pid_n * BLOCK_N) < N_prime, other=-1
            )
            for ki in range(tl.cdiv(D, BLOCK_K)):
                offset_k = k * D + ki * BLOCK_K
    ...
    out_ptr = output + (offsets_n[:, None] * D_prime + tl.arange(0, BLOCK_Dp)[None, :] + pid_dp * BLOCK_Dp)
...
@triton.autotune(
    configs=[
        triton.Config(
            {"BLOCK_DPrime": 16, "BLOCK_NPrime": 16, "BLOCK_D": 32, "PARALLEL_K": 1},
            num_warps=2,
            num_stages=2,
            pre_hook=implicit_gemm_dW_prehook,
        ),
        triton.Config(
            {"BLOCK_DPrime": 16, "BLOCK_NPrime": 32, "BLOCK_D": 16, "PARALLEL_K": 1},
            num_warps=2,
            num_stages=2,
            pre_hook=implicit_gemm_dW_prehook,
        ),
        triton.Config(
            {"BLOCK_DPrime": 32, "BLOCK_NPrime": 32, "BLOCK_D": 32, "PARALLEL_K": 1},
            num_warps=4,
            num_stages=2,
            pre_hook=implicit_gemm_dW_prehook,
        ),
        ...
        triton.Config(
            {"BLOCK_DPrime": 32, "BLOCK_NPrime": 16, "BLOCK_D": 64, "PARALLEL_K": 1},
            num_warps=4,
            num_stages=3,
            pre_hook=implicit_gemm_dW_prehook,
        ),
        triton.Config(
            {"BLOCK_DPrime": 32, "BLOCK_NPrime": 32, "BLOCK_D": 32, "PARALLEL_K": 2},
            num_warps=4,
            num_stages=2,
            pre_hook=implicit_gemm_dW_prehook,
        ),
        triton.Config(
            {"BLOCK_DPrime": 32, "BLOCK_NPrime": 32, "BLOCK_D": 64, "PARALLEL_K": 2},
            num_warps=4,
            num_stages=2,
            pre_hook=implicit_gemm_dW_prehook,
        ),
        triton.Config(
            {"BLOCK_DPrime": 32, "BLOCK_NPrime": 32, "BLOCK_D": 32, "PARALLEL_K": 3},
            num_warps=4,
            num_stages=2,
            pre_hook=implicit_gemm_dW_prehook,
        ),
        triton.Config(
            {"BLOCK_DPrime": 32, "BLOCK_NPrime": 32, "BLOCK_D": 64, "PARALLEL_K": 3},
            num_warps=4,
            num_stages=2,
            pre_hook=implicit_gemm_dW_prehook,
        ),
...
def implicit_gemm_dW_kernel(
    ...
    mask_dweight = (tl.arange(0, BLOCK_D) + (pid_d % blocks_per_d) * BLOCK_D < D)[:, None] & (offset_dp < D_prime)[
        None, :
    ]
    if PARALLEL_K == 1:
