...
def compare_conv3d_subm(coords, dim_in, dim_out, kernel_size):
    ...
    weights = torch.randn(kernel_size ** 3, dim_in, dim_out, device="cuda", dtype=torch.float32) / dim_in**0.5
    indices = ops.idx_gen.gen_conv3d_subm_indices(coords, kernel_size)
    ...
    out_ref = reference_conv3d_subm(feats.float(), indices, weights, kernel_size).half()
